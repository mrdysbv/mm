...
const instance = AsBind.instantiateSync(fs.readFileSync(__dirname + "/../build/untouched.wasm"), {
});
...
const exec = async() => {
  ...
  var _start = new Date().getTime();
  const extractorId = engine.featureExtractorCreate(greyImage.data, image.width, image.height);
  engine.featureExtractorExecute(extractorId);
  var _end = new Date().getTime();
  console.log('exec time: ', _end - _start);

  const imageNum = engine.featureExtractorGetImageNum(extractorId);
  if (DEBUG) {
    for (let i = 0; i < imageNum; i++) {
      const imageData = engine.featureExtractorGetImage(extractorId, i);

      if (imageData.length !== debugContent.imageSets[i].length) console.log("[DEBUG] incorrect image size", i);
      for (let j = 0; j < imageData.length; j++) {
        if (imageData[j] !== debugContent.imageSets[i][j]) {
          console.log("[DEBUG] incorrect image pixel: ", i, j, imageData[j], debugContent.imageSets[i][j]);
          allGood = false;
        }
      }
    }
    ...
    for (let i = 0; i < imageNum; i++) {
      ...
      const featureMap = engine.featureExtractorGetFeatureMap(i);
